...
{   
    ...
    wc access.log | awk '{print $1}'

    printf "\nКоличество запросов каждого типа:\n"
    cat access.log | awk 'length($6)<=10 {print substr($6,2)}' | sort | uniq -c | sort -n # HEAD НА ОДИН МЕНЬШЕ?
    ...
    cat access.log | awk 'int($9/100)==4 {print $1 " " $9 " " $10 " " $7}' | sort -rnk3 | head -5

    printf "\nТоп 5 пользователей по количеству запросов, которые завершились серверной (5ХХ) ошибкой:\n"
    cat access.log | awk 'int($9/100)==5 {print $1}' | sort | uniq -c | sort -rn | head -5
